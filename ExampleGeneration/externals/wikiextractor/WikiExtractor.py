...
from ExampleGeneration.common.context_utils import normalize_paragraph_tag
...
def keepPage(ns, catSet, page):
    ...
    if options.filter_disambig_pages:
        for line in page:
            if filter_disambig_page_pattern.match(line) or "may refer to:" in line:
                return False
...
min_num_of_words_in_paragraph = 4
...
class Extractor(object):
    ...
    def write_output(self, out, text):
        ...
        """
        # write only abstract
        paragraphs = [p for p in text if len(p.split(" ")) > min_num_of_words_in_paragraph]
        for i in range(min(options.n_paragraphs, len(paragraphs))):
            text = paragraphs[i]
            text = normalize_paragraph_tag(BeautifulSoup(text, "html.parser"))
            url = get_url(self.id)
            json_data = {
                'id': f"{self.id}_p{i + 1}",
                'title': self.title,
...
redirectRE = re.compile(r'.*?<redirect title=\"([^>]*?)\" />(?:([^<]*)(<.*?>)?)?')
#                    1     2               3      4
...
def load_templates(file, output_file=None):
    ...
    for page_count, page_data in enumerate(pages_from(file)):
        id, revid, title, ns,catSet, page, redirect = page_data
...
def pages_from(input):
    ...
    for line in input:
        ...
        if tag == 'page':
            ...
            redirect = redirectRE.search(line).group(1)
        elif tag == 'text':
            ...
            if id != last_id:
                yield (id, revid, title, ns,catSet, page, redirect)
                last_id = id
                ns = '0'
            id = None
            revid = None
...
def main(override_args=None):
    ...
    groupO.add_argument("-b", "--bytes", default="1M",
                        ...
                        metavar="n[KMG]")
    groupO.add_argument("-n_p", "--n_paragraphs", type=int, default=1,
                        help="number of first paragraphs to parse")
    ...
    args = parser.parse_args(override_args) if override_args else  parser.parse_args()
    ...
    options.n_paragraphs = args.n_paragraphs
    if args.html:
    ...
    return process_dump(input_file, args.templates, output_path, file_size,
