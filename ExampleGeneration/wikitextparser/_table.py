...
class Table(SubWikiTextWithAttrs):
    ...
    def set_table_title(self, table_title):
    ...
    for attrs_row, row in zip(table_attrs, table_data):
        ...
        for attrs, current_cell in zip(attrs_row, row):
            ...
            try:
                ...
                attrs = re.findall(r'\d+', str(attrs_get(b'colspan', 1)))
                colspan = 1 if len(attrs) == 0 else int(attrs[0])
            if colspan == 0:
            ...
            try:
                ...
                attrs = re.findall(r'\d+', str(attrs_get(b'rowspan', 1)))
                rowspan = 1 if len(attrs) == 0 else int(attrs[0])
            # 13.10
            if rowspan == 0:
